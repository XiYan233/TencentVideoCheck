...
func DownloadCheck(cookie string, vusession string, accessToken string) {
	client := &http.Client{}
	...
	if downCheckStruct.Ret == -2003 {
		...
		insertDB, err := db.Prepare("UPDATE `user` SET `Download`=? WHERE `Cookie`=?")
		if err != nil {
			fmt.Println(err)
		}
		_, err = insertDB.Exec(downCheckStruct.Score, cookie)
		if err != nil {
			fmt.Printf("修改数据出错：%v\n", err)
		}
	}
